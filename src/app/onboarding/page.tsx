...
export default function OnboardingPage() {
  ...
  // Genres derived from liked books
  ...
  // All genres to save = derived + manual
  ...
  // Don't render until we have session (avoid flash)
  if (!session) {
    return (
  ...
  const completeOnboarding = async (skipGenres = false) => {
    ...
    try {
      ...
      // Non-fatal — the session update below is what matters for unlocking the redirect
    }

    // Refresh JWT so middleware sees hasSetUsername: true immediately
    await update({ hasSetUsername: true })
    router.push('/')
  }

  const handleSkip = async () => {
    // Mark as done in JWT without saving anything to DB
    await update({ hasSetUsername: true })
    router.push('/')
  }
