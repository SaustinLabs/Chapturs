...
|||||- **May 1, 2026**: Second-pass audit — corrected profile/blocks count (12), profile/config count (11); noted test API routes; corrected Creator Hub page count from 23→22 and public/auth/reader/admin pages from 54→53; all counts verified accurate across all files. First pass: counts verified accurate (75 pages, 173 API routes, 159 components [92 root + 67 files in subdirectories], 71 lib modules, 89 Prisma models); stale references removed; Work APIs count corrected from "30+" → "42"; Gutenberg file count corrected from "11" → "8". Second pass: Jest tests updated from "1 file" → "13 files (.ts)"; src/ file count corrected from "507" → "518 .tsx/.ts files"; API namespaces clarified as 50 top-level (46 production + 4 test); directory structure expanded to include hooks/, types/, __tests__/, auth-edge.ts; hooks count corrected from "5" → "4 custom React hooks". Third pass: Audiobook APIs corrected from "(3 routes)" → "(1 route)", Reader APIs from "(2 routes)" → "(3 routes)", added 14 undocumented lib modules and 7 subdirectories to CODEBASE_MAP.md, updated Gutenberg Import Pipeline status to "implementation complete", fixed `.squad/wisdom.md` reference in WORKERS.md. Fourth pass: Fixed image safety API line ref in TASKS.md (463→460); confirmed selectionActionRegistry.tsx already documented; removed duplicate lib entries ✅
